...
public class MapUtil {
    ...
    public void setCenter() {
        ...
        String lastLocation = PreferencesUtils.getString(MyApplication.context(), Constants.LAST_LOCATION, "");
        if (!TextUtils.isEmpty(lastLocation)) {
            String[] locationInfo = lastLocation.split(";");
            if (!CommonUtil.isZeroPoint(Double.parseDouble(locationInfo[0]),
                    Double.parseDouble(locationInfo[1]))) {
                LatLng currentLatLng = new LatLng(Double.parseDouble(locationInfo[0]),
                        Double.parseDouble(locationInfo[1]));
                updateStatus(currentLatLng, false);
                return;
            }
